...
template <auto Containers>
void RunAllBenchmarks(const std::vector<size_t> &problem_sizes, const size_t alignment) {
  for (size_t n : problem_sizes) {
    template for (constexpr auto c : Containers) {
      RunBenchmark<typename[: c :], ^^kernels::InvariantMassSequential>(n, alignment, n);
    }
    ...
    template for (constexpr auto c : Containers) {
      RunBenchmark<typename[: c :], ^^kernels::InvariantMassRandom>(n, alignment, n, indices);
    }
...
int main(int argc, char *argv[]) {
  ...
  *output << "benchmark,partitions,problem_size,container_byte_size,time_ratio,"
  ...
  RunAllBenchmarks<containers>(problem_sizes, alignment);
